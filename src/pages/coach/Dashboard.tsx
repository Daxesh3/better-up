...
const CoachDashboard: React.FC = () => {
  ...
  // Demo data
  const stats = [
    { id: 1, name: 'Active Clients', value: 24, icon: <Users size={20} />, change: '+12%', color: 'bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' },
    { id: 2, name: 'Sessions This Week', value: 18, icon: <Calendar size={20} />, change: '+5%', color: 'bg-purple-100 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400' },
    { id: 3, name: 'Unread Messages', value: 7, icon: <MessageSquare size={20} />, change: '-2%', color: 'bg-indigo-100 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400' },
    { id: 4, name: 'Client Growth', value: '23%', icon: <TrendingUp size={20} />, change: '+8%', color: 'bg-green-100 dark:bg-green-900/20 text-green-600 dark:text-green-400' },
  ];
  ...
  const clientAlerts = [
    { id: 1, client: 'Julia Martinez', alert: 'Missed last two sessions', severity: 'high', icon: <AlertCircle size={16} className="text-red-500" /> },
    { id: 2, client: 'David Kim', alert: 'Completed all assigned tasks', severity: 'positive', icon: <CheckCircle size={16} className="text-green-500" /> },
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader 
              ...
              action={<Button variant="outline" size="sm">View Calendar</Button>}
            />
            <CardContent className="px-0 py-0">
              <div className="divide-y divide-gray-200 dark:divide-gray-700">
                {upcomingSessions.map((session) => (
                  <div key={session.id} className="flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors">
                    <div className="flex items-center space-x-3">
                      <div className="flex-shrink-0 h-10 w-10 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center">
                        <span className="font-medium text-indigo-600 dark:text-indigo-400">
                          {session.client.split(' ').map(n => n[0]).join('').toUpperCase()}
                        </span>
                      </div>
                      <div>
        ...
        <div>
          <Card>
            ...
            <CardContent className="space-y-3">
              {clientAlerts.map((alert) => (
                <div 
                  key={alert.id} 
                  className={`flex items-center space-x-3 p-3 rounded-lg border ${getAlertClass(alert.severity)}`}
                >
          ...
          <Card className="mt-6">
            <CardHeader 
              ...
              subtitle="Clients with recent activity"
            />
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center space-x-3">
                  <div className="flex-shrink-0 h-10 w-10 rounded-full bg-gray-200 dark:bg-gray-700">
                    <img
                      src={`https://images.pexels.com/photos/${1000000 + i * 12345}/pexels-photo-${1000000 + i * 12345}.jpeg?auto=compress&cs=tinysrgb&w=150`}
                      alt="Client"
                      className="h-10 w-10 rounded-full object-cover"
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = 'https://images.pexels.com/photos/614810/pexels-photo-614810.jpeg?auto=compress&cs=tinysrgb&w=150';
                      }}
                    />
                  </div>
